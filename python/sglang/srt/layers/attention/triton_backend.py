...
class TritonAttnBackend(AttentionBackend):
    ...
    def _compute_forward_metadata(
        ...
        *,
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            attn_logits = None
            attn_lse = None
        elif use_bound and forward_batch.forward_mode.is_draft_extend(include_v2=True):
            # Captured draft-extend (V1 + V2): constant per-req token layout
            # written into the bound buffers (no graph-illegal host sync). This
            # is the only path that handles draft_extend_v2 — eager v2 (rare:
            # warmup / fallback) falls through to the EXTEND branch below, as in
            # the pre-unification eager body.
            ...
            # Eager draft-extend V1: tree args from spec_info; explicit D2H for
            # max_extend_len instead of an implicit .item() inside
            ...
            # TODO(FIXME): max(num_accept_tokens_cpu) can trigger an invalid
            # Eagle tree; preserved from the pre-unification eager path.
    ...
    def _build_draft_extend_indptrs(
        ...
        """Fill the bound qo/kv indptr buffers for captured draft_extend.

        Returns (qo_indptr, kv_indptr, num_tokens_per_bs).
        """
        # V2 draft-extend fills num_draft_tokens per req; num_steps+1 only
        # equals that when topk == 1.
        num_tokens_per_bs = (
        ...
        if forward_mode.is_draft_extend_v2():
            # seq_lens = prefix + extend; the Triton extend kernel gets extend
            # K/V separately, so kv_indptr/kv_indices cover only the prefix.
            if (
